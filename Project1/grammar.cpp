...
#include <ctime>        // time()...
// time()의 반환 값은 1970년 1월 1일 00:00부터~~~
// 지금까지 건너뜀 시간 (ms) = 1675430097
...
// #include "split_str.h"
#include "../../6-cpp-library-algorithms-2026-20212538-cell-1/Project6/split_str.h"
...
Grammar read_grammar(const string& filename) {
    // aaron.kr에 있는 grammar.txt나 gramar-long.txt 필수
    Grammar ret;
    string line;
...
int nrand(int n) { // Python의 random 모듈에서 같은 함수 있다
    if (n <= 0 || n > RAND_MAX)
        throw domain_error("nrand out od range!");
...
int main(void) {
    ...
    srand(static_cast<unsigned int>(time(0))); // 추가(애런)
